...
var EXCEPTION_UTILS = require('./exception-utils');


function fromXml(filenames, outputDirPath, outputFileName, stringNameKey) {
  if (!Array.isArray(filenames) || filenames.length == 1) {
    throw EXCEPTION_UTILS.buildMalformedInputException(
      "Please pass in an array with at least 2 filenames"
    );
...
function parseStringsXmlFile(filename, stringsWithoutKeyMap, filenames) {
  ...
  if (!strings) {
    throw EXCEPTION_UTILS.buildMalformedInputException(
      "Make sure that all the strings are inside a <resources> tag"
    );
    ...
    for (var i = 0; i < stringsLength; i++) {
      ...
      if (!stringName || !stringValue) {
        throw EXCEPTION_UTILS.buildMalformedInputException(
          "Make sure every string has a name and a valid value"
        );
